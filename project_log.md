...
- **Problema**: Al presionar añadir cita, el badge siempre se inyectaba al principio del input en vez de respetar dónde el usuario había dejado de escribir.
- **Causa**: Al hacer click en el badge flotante (+) fuera del input editable, el `contentEditable` pierde su foco y la DOM `Selection` del navegador colapsa, reseteando la referencia de punto de inserción (Caret Position) al inicio al intentar usar `document.execCommand`.
